...
| `osintx: error: unrecognized arguments: --reset` | в вашей копии старый код (флаг появился в 1.2.3): проверьте `osintx --version` и обновитесь — `git pull`, затем `pip install -e .` (если ставили без `-e`, старая версия осталась в site-packages). Без обновления: удалите `.osintx-data\osintx.session` (и `.session-journal`) вручную и запустите `osintx tgauth` |
| `mtproto:search: TypeError: a TLObject was expected but found something else` | баг версий < 1.2.5 (в запросе был `filter=None`) — обновитесь: `git pull`, затем `pip install -e .`. Проверить, что запросы собираются корректно: `osintx tgcheck` |
| Непонятно, что доступно в MTProto | `osintx tgcheck` — покажет, бот это или ваш аккаунт, и проверит живьём глобальный поиск сообщений и общие группы |
| `BotMethodInvalidError: The API access for bot users is restricted` или `Cannot cast NoneType to any kind of Peer` | вы вошли в MTProto как **бот** (ввели токен бота вместо номера телефона). Обновите проект (в 1.2.3 эти ошибки распознаются) и войдите заново своим аккаунтом: `osintx tgauth --reset`, затем `osintx tgauth` → **номер телефона** `+79991234567` (не токен) → код из Telegram |
