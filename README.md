...
Что реально доступно в вашей MTProto-сессии (аккаунт или бот, работает ли глобальный поиск
и общие группы) — покажет диагностика:

```bash
osintx tgcheck          # python bot.py tgcheck — то же самое
```
...
> ⚠️ **Входить нужно как обычный аккаунт (номер телефона), а не как бот.** Telegram запрещает
> бот-сессиям глобальный поиск сообщений (раньше это выглядело как `BotMethodInvalidError:
> The API access for bot users is restricted (...) GetCommonChatsRequest`) и поиск по сообщениям
> (`Cannot cast NoneType to any kind of Peer`). OsintX теперь распознаёт бот-сессию, пишет об этом
> в отчёте статусом `mtproto:session` и читает то, что ботам разрешено (публичные каналы).
> Пересоздать сессию под своим аккаунтом: `osintx tgauth --reset`, затем `osintx tgauth` — но уже
> с **номером телефона** (`+79991234567`), а не с токеном бота.
