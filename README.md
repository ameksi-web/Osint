...
| `/id <@username\|телефон>` | Telegram-разведка: карточка t.me, подписчики, посты, fragment; **«где писал»** — активность по постам и упоминания через `t.me/s/?q=`; с MTProto — ID, DC, общие чаты, поиск по сообщениям |
...
tests/                # 118 тестов: типы целей, стратегии, калибровка, отчёты, БД, движок, бот, .env,
                      # гео-словарь, инсайты, веб-архив, изменения профиля, VK/MAX, «где писал», реестры
